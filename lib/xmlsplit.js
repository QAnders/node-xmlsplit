    ...
    if (this._tag && this._data) {
      ...
      if (dataChunks.length > 1) {
        ...
        dataChunks.forEach(function(data, index) {
          // This becomes messy as XML allows for spaces, tabs and newlines throughout the XML message
          const tagChk = new RegExp(`^<${this._tagName}[\\s|>]{1}`) // Regex to make sure the chunk we're working on starts with the split tag
          const tagRem = new RegExp(`[^</]<${this._tagName}[\\s|>]{1}`) // RegEx to make sure the split tag is included in the chunk, and there's no closing tag before it
          // Clean up the XML to remove any newlines and replace all tabs with spaces and trim() it to make a true comparison
          if (tagRem.test(data.replace(/(?:\r\n|\r|\n)/g, '').replace(/\t/g, ' ').trim()) && !tagChk.test(data.replace(/(?:\r\n|\r|\n)/g, '').replace(/\t/g, ' ').trim())) {
            // We must have the split tag in the chunk, e.g. <test>, but it can also be <test attr="1">
            // For it to be a nested element, it can't start with the split tag, so <tests> is OK, but not <test> or <test attr="1">
            // If we have e.g. `<root><tests><test>` we will only get `<tests` back, while `<root><book><book>` will give `<book>`
            this._nestedTag = data.replace(/(?:\r\n|\r|\n)/g, '').replace(/\t/g, ' ').trim().split(tagRem).at(0).trim()
            data = data.slice(data.match(tagRem).at(0).length + 1)
          }
          dataChunk += data
          closeTagCount++
          if (index === lastChunkIndex) {
            ...
            if (this._batchSize) {
              this._buffer.push(`${this._nestedTag}${dataChunk}${token}${this._nestedTag?.replace(/</g, '</') || ''}`)
            } else {
